...
import type {
  EsocialProcedureImportClassification,
...
import { EsocialProcedureApplyConfirmDialog } from './EsocialProcedureApplyConfirmDialog';

type Props = {
  open: boolean;
  isLoading: boolean;
  result: IEsocialProcedureImportPreviewResult | null;
  onClose: () => void;
  onApply?: () => void;
  isApplying?: boolean;
};
...
export const EsocialProcedureImportPreviewModal: FC<Props> = ({
  open,
  isLoading,
  result,
  onClose,
  ...
  return (
    ...
    >
      <DialogTitle>
        Prévia da importação — curadoria da Tabela 27
        <Typography variant="body2" color="text.secondary">
          Esta prévia é apenas leitura (dry-run). Nenhuma alteração é gravada no
          banco. A Tabela 27 oficial permanece intocável.
        </Typography>
      </DialogTitle>
              ...
              <TableBody>
                {result.lines.map((line, index) => (
                  <TableRow key={`${line.rowNumber}-${index}`}>
                    ...
                    <TableCell>{line.procedureCode || '—'}</TableCell>
                    <TableCell>{line.procedureNameSnapshot || '—'}</TableCell>
                    <TableCell>
      ...
      <EsocialProcedureApplyConfirmDialog
        open={confirmOpen}
