...
export const RiskQuiContent: FC<{ children?: any } & IUseAddRisk> = ({
  ...
  return (
    <>
      ...
      <SFlex mt={4} gap={8} alignItems="flex-start">
        <Box flex={1}>
          <InputForm
            setValue={setValue}
            ...
            helperText={
              ipvsUnitLabel &&
              riskData.unit &&
              riskData.unit !== ipvsUnitLabel
                ? `Unidade dos OELs: ${riskData.unit}`
                : undefined
            }
          />
        </Box>
        {ipvsMeta?.reference ? (
          <Box width={220}>
            <TextField
              label="Referência"
              value={ipvsMeta.reference}
              size="small"
              fullWidth
              InputProps={{ readOnly: true }}
              helperText={ipvsMeta.origin || undefined}
            />
          </Box>
        ) : null}
      </SFlex>
