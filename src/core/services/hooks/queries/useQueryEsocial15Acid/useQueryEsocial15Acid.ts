...
export const queryEsocial15Acid = async (
  ...
  const response = await api.get<IPaginationResult<IEsocialTable14And15Acid[]>>(
    `${ApiRoutesEnum.ESOCIAL15TABLES}?take=${take}&skip=${skip}&${queries}`,
  );

  return response.data;
};

export function useQueryEsocial15Acid(
  page = 1,
  ...
  const { data, ...result } = useQuery(
    [QueryEnum.ESOCIAL_15, page, { ...pagination, ...query, companyId }],
    () => queryEsocial15Acid(pagination, { ...query, companyId }),
    {
