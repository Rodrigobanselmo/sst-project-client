...
export const queryEvent2220 = async (
  ...
  const response = await api.get<IPaginationResult<IESocialBatch[]>>(
    `${ApiRoutesEnum.ESOCIAL_EVENT_ALL}/batch/${companyId}?take=${take}&skip=${skip}&${queries}`,
  );
...
export function useQueryEventBatch(
  page = 1,
  ...
  const response = {
    data: data?.data || ([] as IESocialBatch[]),
    count: data?.count || 0,
  };
