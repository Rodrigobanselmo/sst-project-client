...
export interface IUpsertINSALUBRIDADEDocumentData {
  id?: string;
  name?: string;
  status?: StatusEnum;
  workspaceId?: string;
...
export async function upsertDocumentData(
  data: IUpsertINSALUBRIDADEDocumentData,
  companyId?: string,
) {
  ...
  const response = await api.post<IRiskGroupData>(
    `${ApiRoutesEnum.DOCUMENT_DATA}/insalubridade`.replace(
      ':companyId',
      companyId,
    ),
    {
...
export function useMutUpsertINSALUBRIDADEDocumentData() {
  const { getCompanyId } = useGetCompanyId();
  const { enqueueSnackbar } = useSnackbar();

  return useMutation(
    async (data: IUpsertINSALUBRIDADEDocumentData) =>
