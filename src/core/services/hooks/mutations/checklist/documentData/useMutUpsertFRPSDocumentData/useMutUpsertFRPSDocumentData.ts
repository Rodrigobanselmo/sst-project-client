...
export interface IUpsertFRPSDocumentData {
  id?: string;
  name?: string;
  status?: StatusEnum;
  workspaceId?: string;
...
export async function upsertDocumentData(
  ...
  const response = await api.post<IRiskGroupData>(
    `${ApiRoutesEnum.DOCUMENT_DATA}/frps`.replace(':companyId', companyId),
    {
      companyId,
      ...data,
    },
...
export function useMutUpsertFRPSDocumentData() {
  const { getCompanyId } = useGetCompanyId();
  const { enqueueSnackbar } = useSnackbar();

  return useMutation(
    async (data: IUpsertFRPSDocumentData) =>
