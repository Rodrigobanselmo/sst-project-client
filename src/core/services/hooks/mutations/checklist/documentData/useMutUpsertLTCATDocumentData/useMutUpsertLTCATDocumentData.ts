...
export interface IUpsertLTCATDocumentData {
  id?: string;
  name?: string;
  status?: StatusEnum;
  workspaceId?: string;
...
export async function upsertDocumentData(
  ...
  const response = await api.post<IRiskGroupData>(
    `${ApiRoutesEnum.DOCUMENT_DATA}/ltcat`.replace(':companyId', companyId),
    {
      companyId,
      ...data,
    },
...
export function useMutUpsertLTCATDocumentData() {
  const { getCompanyId } = useGetCompanyId();
  const { enqueueSnackbar } = useSnackbar();

  return useMutation(
    async (data: IUpsertLTCATDocumentData) =>
