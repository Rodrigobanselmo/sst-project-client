...
export interface ISendEvent2210 {
  companyId: string;
  tpAmb?: number;
  procEmi?: number;
}

export async function mutSend2210(data: ISendEvent2210) {
  const { token } = await refreshToken();
  const response = await api.post(
    `${ApiRoutesEnum.ESOCIAL_EVENT_2210}`,
    {
      ...data,
    },
    {
...
export function useMutSendESocialEvent2210() {
  const { enqueueSnackbar } = useSnackbar();

  return useMutation(async (data: ISendEvent2210) => mutSend2210(data), {
